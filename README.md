...
## ログイン情報
- 本アプリではテスト用にseederで管理者用とテスト用スタッフユーザーを登録しています。  
（開発環境での動作確認用であり、実際の利用時はユーザー自身が登録します）  
- テスト用スタッフユーザーは**メール未認証（email_verified_at = null）で作成**しています。  
(そのため初回ログイン時はメール認証誘導画面へ遷移します)  
※ Featureテストで使用するため、Factory生成ユーザーにはUserFactoryデフォルトのemail_verified_atを付与しており、初回でもメール認証なしでログインします。
